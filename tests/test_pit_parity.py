...
def test_backtest_pit_parity_branch_inits_research_db_before_run_pit_parity():
    ...
    week. Source pin: still must NOT call the full _init_pipeline (that
    guarantee is deliberate and must survive this fix)."""
    import inspect
    import backtest as bt

    src = inspect.getsource(bt)
    idx = src.index("if args.pit_parity:")
    end = src.index("\n    _init_pipeline(args, config)", idx)  # main-path call, after the branch's own return
    branch_src = src[idx:end]
